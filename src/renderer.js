...
import { formatter } from "./downloadFormat.js";
  ...
  modal.querySelectorAll(".format-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        alert(
          `Failed to download as ${format.toUpperCase()}. Error: ${error.message
          }`
        );
...
async function downloadNote(note, format) {
  ...
  switch (format) {
    case "txt":
      formatter.downloadTXT(note, fileName);
      break;
    case "pdf":
      formatter.downloadPDF(note, fileName);
      break;
    case "docx":
      await formatter.downloadDOCX(note, fileName);
      break;
    default:
